<script lang="ts">
	...
	function getClassesForGrade(grade: string) {
		if (!grade) return [];
		return data.allClasses.filter((cls: any) => cls.tags && cls.tags.includes(grade));
	}
	...
	function getInitialSelectedGrade() {
		for (const classId of data.selectedClassIds) {
			...
			const grade = classItem?.tags?.find((tag: string) => tag.startsWith('grade-'));
			if (grade) return grade;
		}

		return data.exam.tags?.find((tag: string) => tag.startsWith('grade-')) ?? '';
	}
	...
	let selectedClassIds = $state<string[]>(getInitialSelectedClassIds(initialSelectedGrade));
...
{#if data.isAdmin}
				...
				<form method="POST" action="?/edit" onsubmit={handleEditSubmit}>
					...
					<div class="modal-body">
						...
						<div class="row g-3">
							...
							<div class="col-12">
								<label class="form-label" for="edit-gradeFilter">Filter classes by grade</label>
								<select class="form-select" id="edit-gradeFilter" value={selectedGrade} onchange={handleGradeChange}>
										...
										{#each getClassesForGrade(selectedGrade) as classItem}
											<div class="col-md-6">
												<div class="form-check">
													...
													/>
													<label class="form-check-label" for="edit-class-{classItem.id}">
														{classItem.title}
													</label>
												</div>
											</div>
	...
	>
		<div class="modal-dialog modal-dialog-centered">
			<div class="modal-content">
				<div class="modal-header">
					<h2 class="modal-title h6" id="confirmEditExamModalLabel">Confirm Edit</h2>
					<button
						type="button"
						class="btn-close"
						data-bs-dismiss="modal"
				...
				<div class="modal-footer">
					...
					<button type="button" class="btn btn-primary" onclick={confirmEdit}>Update Exam</button>
				</div>
			</div>
		</div>
	</div>
	...
	>
		<div class="modal-dialog modal-dialog-centered">
			<div class="modal-content">
				<div class="modal-header">
					<h2 class="modal-title h6" id="confirmDeleteExamModalLabel">Confirm Deletion</h2>
					<button
						type="button"
						class="btn-close"
						data-bs-dismiss="modal"
				...
				<div class="modal-body">
					...
					<p class="text-danger small mb-0">
						This action cannot be undone. All papers and scores associated with this exam will also be deleted.
					</p>
				</div>
				<div class="modal-footer">
					<button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancel</button>
					<form method="POST" action="?/delete" bind:this={deleteFormToSubmit} class="d-inline">
						<button type="button" class="btn btn-danger" onclick={confirmDelete}>Delete Exam</button>
					</form>
				</div>
			</div>
		</div>
